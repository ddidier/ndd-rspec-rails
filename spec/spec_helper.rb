...
RSpec.configure do |config|
  ...
  config.expect_with :rspec do |expectations|
    # This option will default to `true` in RSpec 4. It makes the `description` and `failure_message` of custom
  ...
  # the `mock_with` option here.
  config.mock_with :rspec do |mocks|
    # Prevents you from mocking or stubbing a method that does not exist on a real object. This is generally
  ...
  # Seed global randomization in this process using the `--seed` CLI option. Setting this allows you to use `--seed`
  # to deterministically reproduce test failures related to randomization by passing the same `--seed` value as the
